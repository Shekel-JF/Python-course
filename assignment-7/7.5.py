...
def insertion(tablica, L, R):
    n = L + 1
    while (n < R + 1):
        m = n - 1
        x = tablica[n]
        while ((m >= L) and (tablica[m] > x)):
            tablica[m + 1] = tablica[m]
            m -= 1
        tablica[m + 1] = x
        n += 1
...
def timsort(tablica):
    ...
    for n in range(0, x, y):
        insertion(tablica, n, min((n + y - 1), x - 1))

    while y < x:
        for start in range(0, x, y * 2):
            end = min((start + y * 2 - 1), (x-1))
            if(start + y - 1 >end):
